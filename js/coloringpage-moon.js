...
function draw() {
    ...
    if (mouseIsPressed && distance < planetRad && pdistance < planetRad) {
        g.noStroke();
        g.fill(brushColor);
        g.circle(mouseX, mouseY, width * 0.02); // Smaller brush size proportional to canvas width

        let dotData = {
            x: floor(mouseX),
            y: floor(mouseY),
            r: floor(red(brushColor)),
            g: floor(green(brushColor)),
            b: floor(blue(brushColor)),
        }
        data.dots.push(dotData);
    }
...
function drawPlanetByJSON(data) {
    g.clear();
    g.noStroke();
    g.fill(255);
    g.circle(planetX, planetY, planetRad * 2);
    ...
    for (let i = 0; i < dots.length; i++) {
        let d = dots[i];
        g.fill(d.r, d.g, d.b);
        g.circle(d.x, d.y, width * 0.02); // Smaller brush size proportional to canvas width
    }
